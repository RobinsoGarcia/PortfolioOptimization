...
def get_period(start,T,data):
    ...
    period1 = pd.date_range(start=start, periods=T ,freq='BMs',format='%Y-%m-%d')

    period2 = pd.date_range(start=start, periods=T, freq='BM',format='%Y-%m-%d')
    #print(period1)
    #print(period2)
...
def get_data(t,dt,period,data):
    ...
    #print("NORM:",np.linalg.norm(mu))
    #print("DETERMINANT:",np.linalg.det(Q))
    return Q,mu,p

def backtest_port(port1,data,start,end,t0=2,dt=2,rf=0.0001,T=24,plot_eff=0,report=0) :
    '''

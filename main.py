...
strategies = {'strat_buy_and_hold':prt.strat_buy_and_hold(share_balance = share_balance,V = V_0,buy_h=1),
                'strat_equally_weighted':prt.strat_equally_weighted(share_balance = share_balance,V = V_0),
                'strat_max_Sharpe':prt.strat_max_Sharpe(stocks=stocks,share_balance = share_balance,V = V_0),
                'strat_max_Sharpe_eff':prt.strat_max_Sharpe(share_balance = share_balance,V = V_0,use_eff=1),
...
for x in strategies:
    print(x)
    port = strategies[x]
    w = bkt.backtest_port(port,data,t0=2,dt=2,rf=0.0001,T=24,start= '1/2/2015')
